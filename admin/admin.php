...
$page_nav["Administradores"]["sub"]["Consultar"]["active"] = true;
include("inc/nav.php");
...
else
{
	...
	<div class="modal fade" id="search" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-den="true">
		<form name="" action="admin.php" method="GET" id="">
								...
								<div class="col-md-6">
									<div class="form-group">                    
										...
										<select name='en' id="en" class="form-control">
											<option value="Documento">Documento</option><option value="Nombres">Nombres</option><option value="Apellidos">Apellidos</option><option value="Email">Email</option>
										</select>
									</div>
								</div>              
							</div>  
...
<script type="text/javascript">
	$(document).ready(function() {
		jQuery("#jqgrid").jqGrid({
			...
			if(!$_GET['buscar'])
			{
				echo 'url:"Data/data_admin.php",';
			}else
			{
				echo 'url:"Busqueda/busqueda_admin.php?buscar='.$_GET[buscar].'&en='.$_GET[en].'",';
			}
			?>
			datatype : "json",
			height : 'auto',
			colNames : ['Acciones', 'id', 'Documento', 'Nombres', 'Apellidos', 'Email', 'Contrasena'],
			colModel : [
			{ name : 'act', index:'act', sortable:false }, 
			{ name : 'id', index : 'id',editable : false ,hidden: true ,  align : "center",editable: false}, 
			{ name : 'Documento', index : 'Documento',  align : "center",editable : true }, 
			...
			gridComplete: function(){
				...
				for(var i=0;i < ids.length;i++){
					var cl = ids[i];
					var n= $("#jqgrid").getCell(ids[i],"Documento");
